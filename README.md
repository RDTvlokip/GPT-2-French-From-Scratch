...
- **Measurement harness** (`evaluate.py`) — quantifies generation quality:
  perplexity, 3-gram repetition, distinct-2, and `coherence_len` (tokens before
  topic drift). Reproducible (fixed seed), one dated folder per eval.
...
│   ├── wiki.py                   # Wikipedia corpus scraper
